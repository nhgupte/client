				...
				connect : {
					...
					proxies : [ {
						...
						host : 'localhost', // wherever the data service is
											// running
						port : 9090, // the port that the data service is
										// running on
						changeOrigin : true
					} ],
					options : {
					...
					/*
					 * livereload : { options : { open : true, middleware :
					 * function(connect) { return [ connect.static('.tmp'),
					 * connect() .use( '/bower_components', connect
					...
					livereload : {
						options : {
							...
							middleware : function(connect) {
								...
								middlewares.push(connect().use('/app/styles',
										connect.static('./app/styles')));
...
//################## ADDED configureProxies:server
				grunt.task.run([ 'clean:server', 'wiredep',
						'concurrent:server', 'postcss:server', 'configureProxies:server',
